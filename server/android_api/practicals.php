...
function require_env_admin_token(): void
{
    ...
    // Some servers do not populate `$_SERVER['HTTP_AUTHORIZATION']` reliably for Bearer tokens.
    $auth = '';
    if (function_exists('getallheaders')) {
        $headers = getallheaders();
        if (is_array($headers)) {
            if (isset($headers['Authorization']) && is_string($headers['Authorization'])) $auth = $headers['Authorization'];
            elseif (isset($headers['authorization']) && is_string($headers['authorization'])) $auth = $headers['authorization'];
        }
    }
    if ($auth === '') {

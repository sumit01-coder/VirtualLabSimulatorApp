...
function valid_ip_or_empty($value): string
{
...
function compute_overview(array $state, int $now): array
{
    ...
    return [
        ...
        'error_reqs' => $errs,
        'ts' => $now,
        'config' => ddos_get_config($state),
    ];
}

function compute_config(array $state, int $now): array
{
    ...
    $activeBlocked = 0;
    if (is_array($blocked)) {
        foreach ($blocked as $meta) {
            if (!is_array($meta)) continue;
            $until = (int)($meta['until'] ?? 0);
            if ($until <= 0 || $until > $now) $activeBlocked++;
        }
    }

    return [
        'enabled' => (bool)($cfg['enabled'] ?? true),
        'preset' => (string)($cfg['preset'] ?? 'normal'),
        'limit_10s' => (int)($cfg['limit_10s'] ?? 80),
        'limit_60s' => (int)($cfg['limit_60s'] ?? 300),
        'autoblock_seconds' => (int)($cfg['autoblock_seconds'] ?? 3600),
        'updated_at' => (int)($cfg['updated_at'] ?? 0),
        'active_blocked' => $activeBlocked,
        'tracked_recent' => is_array($recent) ? count($recent) : 0,
...
if ($action === 'block' || $action === 'unblock') {
    ...
    $body = read_json_body();
    $ip = valid_ip_or_empty($body['ip'] ?? '');
    if ($ip === '') {
        ddos_state_unlock($fp);
        json_out(false, 'Invalid ip', null, 400);
    }
    if (ddos_is_whitelisted($ip)) {
        ddos_state_unlock($fp);
        json_out(false, 'Whitelisted IP cannot be modified here', ['ip' => $ip], 409);
    }
...
if ($action === 'update_config' || $action === 'clear_recent' || $action === 'unblock_all') {
    if (($_SERVER['REQUEST_METHOD'] ?? 'GET') !== 'POST') {
        ddos_state_unlock($fp);
        json_out(false, 'POST required', null, 405);
    }
    ...
    if ($action === 'update_config') {
        $cfg = ddos_update_config($state, $body, $now);
        if ($fp !== null) ddos_state_save_locked($fp, $state);
        ddos_state_unlock($fp);
        json_out(true, 'Config updated', $cfg);
    }
    ...
    if ($action === 'clear_recent') {
        ...
        if ($fp !== null) ddos_state_save_locked($fp, $state);
        ddos_state_unlock($fp);
        json_out(true, 'Recent history cleared', ['ts' => $now]);
    }

    if ($action === 'unblock_all') {
        $keepWhitelist = !array_key_exists('keep_whitelist', $body)
            || filter_var($body['keep_whitelist'], FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE) !== false;
        $blocked = $state['blocked'] ?? [];
        $nextBlocked = [];
        $removed = 0;
        if (is_array($blocked)) {
            foreach ($blocked as $blockedIp => $meta) {
                if ($keepWhitelist && ddos_is_whitelisted((string)$blockedIp)) {
                    $nextBlocked[$blockedIp] = $meta;
                } else {
                    $removed++;
                }
            }
        ...
        if ($fp !== null) ddos_state_save_locked($fp, $state);
        ddos_state_unlock($fp);
        json_out(true, 'All blocks cleared', ['removed' => $removed, 'kept_whitelist' => $keepWhitelist]);
    }
}
...
else if ($action === 'config') $data = compute_config($state, $now);
else {
    ddos_state_unlock($fp);

...
try {
    ...
    $stmt = $pdo->prepare("SELECT id, username, email, password, role, department_id FROM admins WHERE username = ? OR email = ? LIMIT 1");
    ...
    if ($admin && password_verify($password, $admin['password'])) {
        ...
        if (!$admin) {
            ...
            send_json_response(false, 'Account not found. Please contact the Virtual Lab admin.');
        } else {
            header('HTTP/1.0 401 Unauthorized');
            send_json_response(false, 'Incorrect password');
        }
    }
} catch (PDOException $e) {
